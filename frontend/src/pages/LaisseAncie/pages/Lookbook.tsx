    ...
    <div className="p-8 lg:p-12 max-w-[1500px] mx-auto">
      <header className="flex items-end justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">款式总览 — 按创作模式分类 · 点击行查看设计提案</p>
        </div>
              ...
              {items.map((p) => (
                <tr key={p.id} className="border-b border-gray-200 hover:bg-primary-50/40 cursor-pointer transition-colors"
                  ...
                  <td className="px-3 py-3 max-w-[260px]">
                    <div className="font-medium text-gray-900 truncate" title={p.title}>{p.title || "(untitled)"}</div>
                    {(p.sections?.themeNarrative || p.description) && (
                      <div className="text-[11px] text-gray-500 truncate">{p.sections?.themeNarrative || p.description}</div>
                    )}
                    {p.category && p.category !== p.mode && <div className="text-[10px] text-primary-600 mt-0.5">{p.category}</div>}
                  </td>
                  ...
                  <td className="px-3 py-3" onClick={(e) => e.stopPropagation()}>
                    <StatusSelect product={p} onChange={(s) => handleStatusChange(p.id, s)} />
                  </td>
...
function StatusSelect({ product, onChange }: { product: Product; onChange: (s: ProductStatus) => void }) {
  return (
    <select
      ...
      className={`text-[11px] font-medium rounded-md border px-2 py-1 max-w-[130px] focus:outline-none focus:border-primary-500 transition-colors ${
        product.status === "live"
    ...
    >
      {STATUS_FLOW.map((s) => (
        <option key={s} value={s}>
          {STATUS_LABEL[s]}{s === product.status ? " ✓" : ""}
        </option>
      ))}
    </select>
  );
