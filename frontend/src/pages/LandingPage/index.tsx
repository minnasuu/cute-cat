    ...
    <div className="min-h-screen bg-surface text-text-primary selection:bg-primary-100 selection:text-primary-900">
      ...
      <section className="relative pt-20 pb-20 md:pt-32 md:pb-40 overflow-hidden">
        ...
        <div className="mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 px-6">
            ...
            <Link to="/community" className="w-full sm:w-auto px-10 py-4 text-lg font-bold bg-surface-secondary text-text-primary rounded-2xl border border-border-strong hover:bg-surface-tertiary transition-all">浏览社区作品</Link>
          </div>
        </div>
      </section>
      ...
      <section id="workspaces" className="py-24 md:py-32">
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-16 text-center">
            <p className="text-sm font-bold text-primary-500 uppercase tracking-widest mb-4">工作台</p>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6">每条赛道,一条专属流水线</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {WORKSPACES.map((w, i) => (
              <div key={i} className={`rounded-[24px] border p-6 transition-all ${w.active ? 'border-primary-200 bg-primary-50/50 shadow-lg' : 'border-border bg-surface opacity-70'}`}>
                <div className="text-4xl mb-4">{w.icon}</div>
                <h3 className="text-xl font-black text-text-primary mb-2">{w.name}</h3>
                <p className="text-sm text-text-secondary">{w.desc}</p>
                {w.active && <span className="inline-block mt-4 px-3 py-1 bg-primary-500 text-white text-xs font-bold rounded-full">当前开放</span>}
                {!w.active && <span className="inline-block mt-4 px-3 py-1 bg-surface-secondary text-text-tertiary text-xs font-bold rounded-full">即将推出</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="community" className="py-24 md:py-32">
        <div className="max-w-6xl mx-auto px-6 text-center">
          <p className="text-sm font-bold text-accent-500 uppercase tracking-widest mb-4">社区</p>
          <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6">从工作台到舞台</h2>
          <p className="text-lg text-text-secondary max-w-2xl mx-auto mb-10">
            将设计作品一键公开,在社区展示、获得反馈,与其他创作者交流灵感。
          </p>
          <Link to="/community" className="inline-flex items-center gap-2 px-8 py-4 text-lg font-bold bg-accent-500 text-white rounded-2xl hover:bg-accent-600 transition-all">
      ...
      <section className="py-24 md:py-32 bg-surface-secondary/30">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link to="/login" className="w-full sm:w-auto px-10 py-4 text-lg font-bold bg-surface text-text-primary rounded-2xl border border-border-strong hover:bg-surface-tertiary transition-all">登录</Link>
          </div>
        </div>
      </section>
      ...
      <footer className="py-6 border-t border-border">
        <div className="mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-6 text-sm text-text-tertiary">
            <Link to="/community" className="hover:text-text-primary transition-colors">社区</Link>
            <Link to="/login" className="hover:text-text-primary transition-colors">登录</Link>
            <Link to="/register" className="hover:text-text-primary transition-colors">注册</Link>
          </div>
