...
你的任务：把上游的「简历内容（Markdown）」渲染成一个可直接打开的单文件 HTML 简历。
...
## 输入说明
- 上游会提供结构化的 Markdown 简历内容（# 与 ## 标题 + 列表）。
`;
...
module.exports = async function runResumeHtmlEngineer(ctx) {
  ...
  const result = await runWithAIStream('resume-html-engineer', ctx, SYSTEM_PROMPT, userText, {
    maxTokens: 16384,
